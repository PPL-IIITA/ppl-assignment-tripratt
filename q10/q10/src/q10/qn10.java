...
public class qn10 {
	...
	public void fnc() throws FileNotFoundException, IOException {
		...
		in = new FileReader("C:\\Users\\dell\\Desktop\\q10\\log_g.txt");
		in1 = new FileReader("C:\\Users\\dell\\Desktop\\q10\\log_b.txt");
		...
		for (int i = 0; i < 12; i++) {
			if (cateG[i] == 'c') {
				...
				BufferedReader br = new BufferedReader(new FileReader("C:\\Users\\dell\\Desktop\\q10\\log_g.txt"));
				line = br.readLine();
		...
		BufferedReader br1 = new BufferedReader(new FileReader("C:\\Users\\dell\\Desktop\\q10\\log_b.txt"));
		for (int i = 0; i < 58; i++) {
			if (cateB[i] == 'm') {
				b[i] = new miserBoy();
		...
		int line1;
		int x = 0, y = 0;
		BufferedReader gr = new BufferedReader(new FileReader("C:\\Users\\dell\\Desktop\\q10\\nameGirl.txt"));
		BufferedReader gr1 = new BufferedReader(new FileReader("C:\\Users\\dell\\Desktop\\q10\\nameBoy.txt"));
		...
		for (int z = 0; z < 58; z++) {
			arr3[z] = b[z].intelligenceLevel;
		}
		for (int z = 0; z < 58; z++) {
			arr4[z] = b[z].attractiveness;
		}
		fn<girl> ob = new fn<girl>();
		ob.fnc(g, 6, arr1);
		fn_r<boy> ob2 = new fn_r<boy>();
		int rec = ob2.fnc();
		for (int z = 0; z < 6; z++) {
			if (g[z].criteria == 'r') {
				fn<boy> ob1 = new fn<boy>();
				ob1.fnc(b, 6, arr2);
				if (b[rec].isCommitted == 0 && b[rec].budget > g[z].maintenanceCost) {
					b[rec].isCommitted = 1;
					ind_b[z] = rec;
					g[z].nB = b[rec].name;
				} else {
		...
		out2 = new FileWriter("C:\\Users\\dell\\Desktop\\q10\\couple.txt");
		for (int z = 0; z < 12; z++) {
		...
		for (int i = 11; i > 11 - ki; i--) {
			...
			while (happy[i] != c[ji].happiness)
				ji++;
			System.out.println(c[ji].nGirl + " " + c[ji].nBoy + " " + happy[i]);
		}
		...
		for (int i = 11; i > 11 - ki; i--) {
			...
			while (co[i] != c[p].compatibility)
				p++;
			System.out.println(c[p].nGirl + " " + c[p].nBoy);
		}
	}
